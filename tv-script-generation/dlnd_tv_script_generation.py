...
def get_batches(int_text, batch_size, seq_length):
    ...
    # Reshape input and targets 
    input_ = int_text[: n_batches * words_per_batch]
    input_ = np.array(input_).reshape((batch_size, -1))
    
    targets_ = int_text[1: n_batches * words_per_batch]
    targets_.append(int_text[0])
    targets_ = np.array(targets_).reshape((batch_size, -1))
    
    # Apply to batches
    batches = []
    
    for n in range(0, input_.shape[1], seq_length):
        x = input_[:, n:n + seq_length].tolist()
        y = targets_[:, n:n + seq_length].tolist()
        batches.append([x, y])
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    i = np.random.choice(len(int_to_vocab), 1, p=probabilities)[0]
